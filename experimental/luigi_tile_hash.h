...
void _UIRenderCommandRun(UIRenderCommand command, uint32_t *_bits, int width, int height) {
	if (command.type == UI_RENDER_COMMAND_BLOCK) {
		for (int line = command.rectangle.t; line < command.rectangle.b; line++) {
			uint32_t *bits = _bits + line * width + command.rectangle.l;
			int count = UI_RECT_WIDTH(command.rectangle);
		...
		const uint8_t *data = (const uint8_t *) _uiFont + command.character * 16;

		for (int i = command.rectangle.t; i < command.rectangle.b; i++) {
			uint32_t *bits = _bits + i * width + command.rectangle.l;
			uint8_t byte = data[i - command.y];

			for (int j = command.rectangle.l; j < command.rectangle.r; j++) {
				if (byte & (1 << (j - command.x))) {
					*bits = command.color;
				}
...
void _UIWindowComputeTileHashes(UIWindow *window, UIPainter *painter) {
	...
	for (uintptr_t i = 0; i < tileCountX * tileCountY; i++) {
		window->tilesNew[i] = 0xCBF29CE484222325;
	}

	for (uintptr_t i = 0; i < painter->commandCount; i++) {
		UIRenderCommand *command = painter->commands + i;
		command->rectangle = UIRectangleIntersection(command->rectangle, UI_RECT_2S(window->width, window->height));

		uint64_t h = 0xCBF29CE484222325;

		// If we're rendering a command that behaves independent of the coordinates, 
		// then we only want to consider the intersection of the its rectangle with each tile.
		uintptr_t start = command->type < 0x80 ? sizeof(UIRectangle) : 0;

		for (uintptr_t j = start; j < sizeof(UIRenderCommand); j++) {
			h = (h ^ ((uint8_t *) command)[j]) * 0x100000001B3;
		}

		for (int y = command->rectangle.t / UI_TILE_HEIGHT; y <= (command->rectangle.b - 1) / UI_TILE_HEIGHT; y++) {
			for (int x = command->rectangle.l / UI_TILE_WIDTH; x <= (command->rectangle.r - 1) / UI_TILE_WIDTH; x++) {
				// TODO Opaque optimisation - don't add the hash of this command if we've
				// 	already found an opaque command for this tile.

				uint64_t *hash = window->tilesNew + (y * tileCountX + x);

				for (uintptr_t j = 0; j < sizeof(h); j++) {
					*hash = (*hash ^ ((uint8_t *) &h)[j]) * 0x100000001B3;
				}

				if (command->type < 0x80) {
					UIRectangle tile = UI_RECT_4(x * UI_TILE_WIDTH, (x + 1) * UI_TILE_WIDTH, 
							y * UI_TILE_HEIGHT, (y + 1) * UI_TILE_HEIGHT);
					UIRectangle intersection = UIRectangleIntersection(tile, command->rectangle);

					for (uintptr_t j = 0; j < sizeof(UIRectangle); j++) {
						*hash = (*hash ^ ((uint8_t *) &intersection)[j]) * 0x100000001B3;
					}
				}
			}
		}
...
void _UIWindowRasterizeTileCluster(UIWindow *window, UIPainter *painter, UIRectangle tile, bool withDebugOutline) {
	...
	while (i < painter->commandCount) {
		UIRenderCommand command = painter->commands[i++];
		command.rectangle = UIRectangleIntersection(command.rectangle, tile);
		if (!UI_RECT_VALID(command.rectangle)) continue;
		_UIRenderCommandRun(command, window->bits, window->width, window->height);
	}
...
void _UIWindowFindAndPaintTileClusters(UIWindow *window, UIPainter *painter, bool withDebugOutline) {
	size_t tileCountX = (window->width + UI_TILE_WIDTH - 1) / UI_TILE_WIDTH;
	...
	for (int y = window->repaint.t / UI_TILE_HEIGHT; y <= (window->repaint.b - 1) / UI_TILE_HEIGHT; y++) {
		...
		while (x <= (window->repaint.r - 1) / UI_TILE_WIDTH) {
			...
			if (window->tilesOld[i] == window->tilesNew[i]) {
				x++;
				continue;
			}
			...
			while (true) {
				if (x > (window->repaint.r - 1) / UI_TILE_WIDTH
						|| window->tilesOld[i] == window->tilesNew[i]) {
					break;
				}
			...
			// TODO Expanding the cluster vertically.

			_UIWindowRasterizeTileCluster(window, painter, UIRectangleIntersection(cluster, window->repaint), withDebugOutline);
		}
	}
}
		...
		if (_UIDestroy(&window->e)) {
			*link = next;
			...
			if (UI_RECT_VALID(window->repaint)) {
				...
				_UIWindowComputeTileHashes(window, &painter);

				int start, end;
				window->bits = bits2;
				start = clock();
				_UIWindowFindAndPaintTileClusters(window, &painter, false);
				end = clock();
				float tileHash = (float) (end - start) / CLOCKS_PER_SEC;
				window->bits = bits1;
				start = clock();
				_UIWindowRasterizeTileCluster(window, &painter, window->repaint, false);
				end = clock();
				float boundingRect = (float) (end - start) / CLOCKS_PER_SEC;
				printf("tile-hash: %.3fms      bounding-rect: %.3fms        ", tileHash * 1000, boundingRect * 1000);
				if (tileHash > boundingRect + 1e-5) printf("tile-hash slower by %fx\n", (tileHash - boundingRect) / boundingRect);
				// else printf("bounding-rect slower by %fx\n", (boundingRect - tileHash) / tileHash);
				else printf("\n");
				memcpy(window->bits, bits2, window->width * window->height * 4);
				_UIWindowFindAndPaintTileClusters(window, &painter, true);
...

				{
					...
					for (uintptr_t i = 0; i < tileCountX * tileCountY; i++) {
						if (window->tilesNew[i] != 0xCBF29CE484222325) {
							window->tilesOld[i] = window->tilesNew[i];
						}
					}
				}
...
bool _UIProcessEvent(XEvent *event) {
	...
	if (event->type == ClientMessage && (Atom) event->xclient.data.l[0] == ui.windowClosedID) {
		...
		if (window->width != event->xconfigure.width || window->height != event->xconfigure.height) {
			...
			size_t tileCount = (window->width + UI_TILE_WIDTH - 1) / UI_TILE_WIDTH * (window->height + UI_TILE_HEIGHT - 1) / UI_TILE_HEIGHT;
			window->tilesOld = (uint64_t *) UI_REALLOC(window->tilesOld, tileCount * sizeof(uint64_t));
			window->tilesNew = (uint64_t *) UI_REALLOC(window->tilesNew, tileCount * sizeof(uint64_t));
			for (uintptr_t i = 0; i < tileCount; i++) window->tilesOld[i] = 0;
