...
#define UI_TILE_WIDTH (128)
#define UI_TILE_HEIGHT (128)

typedef struct UIRenderCommand {
	UIRectangle rectangle;

#define UI_RENDER_COMMAND_BLOCK  (0x00)
#define UI_RENDER_COMMAND_INVERT (0x01)
#define UI_RENDER_COMMAND_GLYPH  (0x80)
	uint8_t type;
	bool opaque;
	uint32_t x, y;
	uint32_t color;
...
typedef struct UIQueuedTileCluster {
	struct UIWindow *window;
	struct UIPainter *painter;
	UIRectangle rectangle;
	bool withDebugOutline;
} UIQueuedTileCluster;
...
typedef struct UIPainter {
	...
	size_t commandCount, commandsAllocated;
} UIPainter;
...
#define UI_WINDOW_CENTER_IN_OWNER (1 << 2)
	...
	uint32_t *bits;
	uint32_t *bits2; // TODO Temporary!
	uint32_t *bits3; // TODO Temporary!
	int width, height;
...
struct {
	...
	UIQueuedTileCluster *queuedTileClusters;
	volatile size_t queuedTileClusterCount;
	volatile size_t queuedTileClustersPosition;
	volatile size_t queuedTileClustersCompleted;
	volatile bool queuedTileClustersRunning;
	size_t queuedTileClustersAllocated;
	pthread_cond_t renderEvent;
...
void UIElementRepaint(UIElement *element, UIRectangle *region) {
	...
	size_t tileCountX = (element->window->width + UI_TILE_WIDTH - 1) / UI_TILE_WIDTH;

	for (int y = r.t / UI_TILE_HEIGHT; y <= (r.b - 1) / UI_TILE_HEIGHT; y++) {
		for (int x = r.l / UI_TILE_WIDTH; x <= (r.r - 1) / UI_TILE_WIDTH; x++) {
			int i = y * tileCountX + x;
			element->window->tilesToRepaint[i / 8] |= 1 << (i & 7);
		}
	}

	if (UI_RECT_VALID(element->window->repaint)) {
		element->window->repaint = UIRectangleBounding(element->window->repaint, r);
	} else {
		element->window->repaint = r;
	}
}
...
void _UIRenderCommandRun(const UIRenderCommand *command, UIRectangle rectangle, uint32_t *_bits, int width, int height) {
	if (command->type == UI_RENDER_COMMAND_BLOCK) {
		for (int line = rectangle.t; line < rectangle.b; line++) {
			uint32_t *bits = _bits + line * width + rectangle.l;
	...
	} else if (command->type == UI_RENDER_COMMAND_GLYPH) {
		int x = command->x;

		for (uintptr_t i = 0; i < command->characterCount; i++, x += 9) {
			const uint8_t *data = (const uint8_t *) _uiFont + command->characters[i] * 16;
			UIRectangle r = UIRectangleIntersection(rectangle, UI_RECT_4(x, x + 8, command->y, command->y + 16));
			if (!UI_RECT_VALID(r)) continue;

			for (int i = r.t; i < r.b; i++) {
				uint32_t *bits = _bits + i * width + r.l;
				uint8_t byte = data[i - command->y];
	...
	} else if (command->type == UI_RENDER_COMMAND_INVERT) {
		for (int line = rectangle.t; line < rectangle.b; line++) {
			uint32_t *bits = _bits + line * width + rectangle.l;
			int count = UI_RECT_WIDTH(rectangle);
...
void _UIRenderCommandAdd(UIPainter *painter, UIRenderCommand *command) {
	command->rectangle = UIRectangleIntersection(painter->clip, command->rectangle);

	if (!UI_RECT_VALID(command->rectangle)) {
		return;
	}

	if (painter->commandCount + 1 >= painter->commandsAllocated) {
		painter->commandsAllocated = (painter->commandCount + 1) * 2;
		painter->commands = (UIRenderCommand *) UI_REALLOC(painter->commands, sizeof(UIRenderCommand) * painter->commandsAllocated);
	}

	painter->commands[painter->commandCount++] = *command;
}

void UIDrawBlock(UIPainter *painter, UIRectangle rectangle, uint32_t color) {
	UIRenderCommand command = { 0 };
	command.rectangle = rectangle;
	command.type = UI_RENDER_COMMAND_BLOCK;
	command.color = color;
	command.opaque = true;
	_UIRenderCommandAdd(painter, &command);
}
...
void UIDrawInvert(UIPainter *painter, UIRectangle rectangle) {
	UIRenderCommand command = { 0 };
	command.rectangle = rectangle;
	command.type = UI_RENDER_COMMAND_INVERT;
	_UIRenderCommandAdd(painter, &command);
}
...
void UIDrawGlyph(UIPainter *painter, int x, int y, int c, uint32_t color) {
	UIRenderCommand *last = painter->commandCount ? (painter->commands + painter->commandCount - 1) : NULL;

	if (last && last->type == UI_RENDER_COMMAND_GLYPH && last->characterCount != sizeof(last->characters)
			&& last->x + last->characterCount * 9 == (uint32_t) x && last->y == (uint32_t) y && last->color == color) {
		UIRectangle rectangle = UI_RECT_4(x, x + 8, y, y + 16);
		last->rectangle = UIRectangleIntersection(UIRectangleBounding(last->rectangle, rectangle), painter->clip);
		last->characters[last->characterCount] = (c < 0 || c > 127) ? '?' : c;
		last->characterCount++;
	} else {
		UIRenderCommand command = { 0 };
		command.rectangle = UI_RECT_4(x, x + 8, y, y + 16);
		command.type = UI_RENDER_COMMAND_GLYPH;
		command.color = color;
		command.characters[0] = (c < 0 || c > 127) ? '?' : c;
		command.characterCount = 1;
		command.x = x;
		command.y = y;
		_UIRenderCommandAdd(painter, &command);
	}
...
void _UIWindowRasterizeTileCluster(UIWindow *window, UIPainter *painter, UIRectangle tile, bool withDebugOutline) {
	uintptr_t i = painter->commandCount;

	while (i > 0) {
		i--;

		if (painter->commands[i].opaque && UIRectangleEquals(UIRectangleIntersection(painter->commands[i].rectangle, tile), tile)) {
			break;
		}
	}

	while (i < painter->commandCount) {
		UIRenderCommand *command = painter->commands + (i++);
		UIRectangle rectangle = UIRectangleIntersection(command->rectangle, tile);
		if (!UI_RECT_VALID(rectangle)) continue;
		_UIRenderCommandRun(command, rectangle, window->bits, window->width, window->height);
	}
	...
	if (withDebugOutline) {
		...
		command.rectangle = UIRectangleIntersection(UI_RECT_2S(window->width, window->height), UI_RECT_4(tile.l, tile.r, tile.t, tile.t + 4));

		if (UI_RECT_VALID(command.rectangle)) {
			_UIRenderCommandRun(&command, command.rectangle, window->bits, window->width, window->height);
		}

		command.rectangle = UIRectangleIntersection(UI_RECT_2S(window->width, window->height), UI_RECT_4(tile.l, tile.r, tile.b - 4, tile.b));

		if (UI_RECT_VALID(command.rectangle)) {
			_UIRenderCommandRun(&command, command.rectangle, window->bits, window->width, window->height);
		}

		command.rectangle = UIRectangleIntersection(UI_RECT_2S(window->width, window->height), UI_RECT_4(tile.l, tile.l + 4, tile.t, tile.b));

		if (UI_RECT_VALID(command.rectangle)) {
			_UIRenderCommandRun(&command, command.rectangle, window->bits, window->width, window->height);
		}

		command.rectangle = UIRectangleIntersection(UI_RECT_2S(window->width, window->height), UI_RECT_4(tile.r - 4, tile.r, tile.t, tile.b));
...
void _UIWindowFindTileClusters(UIWindow *window, UIPainter *painter, bool withDebugOutline) {
	size_t tileCountX = (window->width + UI_TILE_WIDTH - 1) / UI_TILE_WIDTH;

	for (int y = window->repaint.t / UI_TILE_HEIGHT; y <= (window->repaint.b - 1) / UI_TILE_HEIGHT; y++) {
		int x = window->repaint.l / UI_TILE_WIDTH;
		int xs = x;

		while (x <= (window->repaint.r - 1) / UI_TILE_WIDTH) {
			int i = y * tileCountX + x;

			if (~window->tilesToRepaint[i / 8] & (1 << (i & 7))) {
				x++;
				continue;
			}

			UIRectangle cluster = UI_RECT_4(x * UI_TILE_WIDTH, x * UI_TILE_WIDTH, 
					y * UI_TILE_HEIGHT, (y + 1) * UI_TILE_HEIGHT);

#if 1
			(void) xs;
			window->tilesToRepaint[i / 8] &= ~(1 << (i & 7));
			cluster.r += UI_TILE_WIDTH;
			x++;
#else
			while (true) {
				if (x > (window->repaint.r - 1) / UI_TILE_WIDTH
						|| (~window->tilesToRepaint[i / 8] & (1 << (i & 7)))) {
					break;
				}
			...
			for (int y0 = y + 1; y0 <= (window->repaint.b - 1) / UI_TILE_HEIGHT; y0++) {
				int i = y0 * tileCountX + xs;

				for (int x0 = xs; x0 < x; x0++, i++) {
					if (~window->tilesToRepaint[i / 8] & (1 << (i & 7))) {
						goto stopExpandingVertically;
					}
				}
				...
				for (int x0 = xs; x0 < x; x0++, i++) {
					window->tilesToRepaint[i / 8] &= ~(1 << (i & 7));
					cluster.b += UI_TILE_HEIGHT;
				}

				continue;
				stopExpandingVertically:;
				break;
			}
#endif

			if (ui.queuedTileClusterCount + 1 >= ui.queuedTileClustersAllocated) {
				ui.queuedTileClustersAllocated = (ui.queuedTileClusterCount + 1) * 2;
				ui.queuedTileClusters = (UIQueuedTileCluster *) UI_REALLOC(ui.queuedTileClusters, 
						sizeof(UIQueuedTileCluster) * ui.queuedTileClustersAllocated);
			}

			UIQueuedTileCluster *entry = ui.queuedTileClusters + (ui.queuedTileClusterCount++);

			entry->window = window;
			entry->painter = painter;
			entry->rectangle = UIRectangleIntersection(cluster, window->repaint);
			entry->withDebugOutline = withDebugOutline;
		}
	}
}

void _UIRenderThreadProcess() {
	while (true) {
		uintptr_t i = __sync_fetch_and_add(&ui.queuedTileClustersPosition, 1);

		if (i < ui.queuedTileClusterCount) {
			UIQueuedTileCluster *entry = ui.queuedTileClusters + i;
			_UIWindowRasterizeTileCluster(entry->window, entry->painter, entry->rectangle, entry->withDebugOutline);
			__sync_fetch_and_add(&ui.queuedTileClustersCompleted, 1);
		} else {
			break;
		}
	}
...
void *_UIRenderThread(void *_unused) {
	while (true) {
		if (ui.queuedTileClustersRunning) {
			_UIRenderThreadProcess();
		} else {
#if 0
...
#endif
			pthread_mutex_lock(&ui.renderMutex);
			pthread_cond_wait(&ui.renderEvent, &ui.renderMutex);
			pthread_mutex_unlock(&ui.renderMutex);
		}
	}
	...
	while (window) {
		...
		if (_UIDestroy(&window->e)) {
			*link = next;
			...
			if (UI_RECT_VALID(window->repaint)) {
#ifdef __cplusplus
				UIPainter painter = {};
#else
				UIPainter painter = { 0 };
#endif
				painter.clip = UIRectangleIntersection(window->repaint, UI_RECT_2S(window->width, window->height));
				_UIElementPaint(&window->e, &painter);

				uint32_t *bits1 = window->bits;

				memcpy(window->bits, window->bits2, window->width * window->height * 4);
				window->bits = window->bits2;
				_UIWindowRasterizeTileCluster(window, &painter, window->repaint, false);

				window->bits = window->bits3;
				clock_t start = clock();
				_UIWindowRasterizeTileCluster(window, &painter, window->repaint, true);
				clock_t end = clock();
				float boundingRect = (float) (end - start) / CLOCKS_PER_SEC;
				printf("bounding-rect: %.3fms        ", boundingRect * 1000);

				window->bits = bits1;
				start = clock();
				ui.queuedTileClusterCount = 0;
				_UIWindowFindTileClusters(window, &painter, true);
				ui.queuedTileClustersPosition = 0;
				ui.queuedTileClustersCompleted = 0;
				ui.queuedTileClustersRunning = true;
				pthread_mutex_lock(&ui.renderMutex);
				pthread_cond_broadcast(&ui.renderEvent);
				pthread_mutex_unlock(&ui.renderMutex);
				_UIRenderThreadProcess();
				while (ui.queuedTileClustersCompleted < ui.queuedTileClusterCount);
				ui.queuedTileClustersRunning = false;
				end = clock();
				float tileBits = (float) (end - start) / CLOCKS_PER_SEC;
				printf("tile-bits: %.3fms        ", tileBits * 1000);

				size_t glyphCommandCount = 0;

				for (uintptr_t i = 0; i < painter.commandCount; i++) {
					if (painter.commands[i].type == UI_RENDER_COMMAND_GLYPH) {
						glyphCommandCount++;
					}
				}

				printf("%ld/%ld glyphs", glyphCommandCount, painter.commandCount);

				printf("\n");

				UI_FREE(painter.commands);
				window->repaint = UI_RECT_2S(window->width, window->height); 
				_UIWindowEndPaint(window, &painter);
				window->repaint = UI_RECT_1(0);
			}
		}
...
int _UIInspectorTableMessage(UIElement *element, UIMessage message, int di, void *dp) {
	...
	if (message == UI_MSG_TABLE_GET_ITEM) {
		...
		UIPainter painter = { 0 };
		window->repaint = window->e.bounds;
...
void UIInitialise() {
	...
	pthread_cond_init(&ui.renderEvent, NULL);
	pthread_mutex_init(&ui.renderMutex, NULL);

	for (int i = 0; i < sysconf(_SC_NPROCESSORS_CONF) - 1; i++) {
		pthread_t thread;
		pthread_create(&thread, NULL, _UIRenderThread, NULL);
	}
}
...
void _UIWindowEndPaint(UIWindow *window, UIPainter *painter) {
	...
	XPutImage(ui.display, window->window, DefaultGC(ui.display, 0), window->image, 
		UI_RECT_TOP_LEFT(window->repaint), UI_RECT_TOP_LEFT(window->repaint),
		UI_RECT_SIZE(window->repaint));
}
...
bool _UIProcessEvent(XEvent *event) {
	...
	if (event->type == ClientMessage && (Atom) event->xclient.data.l[0] == ui.windowClosedID) {
		...
		if (window->width != event->xconfigure.width || window->height != event->xconfigure.height) {
			...
			window->bits2 = (uint32_t *) UI_REALLOC(window->bits2, window->width * window->height * 4);
			window->bits3 = (uint32_t *) UI_REALLOC(window->bits3, window->width * window->height * 4);

			size_t tileCountX = (window->width + UI_TILE_WIDTH - 1) / UI_TILE_WIDTH;
			size_t tileCountY = (window->height + UI_TILE_HEIGHT - 1) / UI_TILE_HEIGHT;
			window->tilesToRepaint = (uint8_t *) UI_REALLOC(window->tilesToRepaint, (tileCountX * tileCountY + 7) / 8);
...
void _UIWindowEndPaint(UIWindow *window, UIPainter *painter) {
	...
	info.biPlanes = 1, info.biBitCount = 32;
	StretchDIBits(dc, 
		UI_RECT_TOP_LEFT(window->repaint), UI_RECT_SIZE(window->repaint), 
		window->repaint.l, window->repaint.b + 1, 
		UI_RECT_WIDTH(window->repaint), -UI_RECT_HEIGHT(window->repaint),
		window->bits, (BITMAPINFO *) &info, DIB_RGB_COLORS, SRCCOPY);
